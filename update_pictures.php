...
try {
    ...
    if ($count > 0) {
        // Set picture for imported students
        $sql = "UPDATE student_info SET picture = 'pix/anonymous.jpg' WHERE student_number IN (SELECT student_id FROM temp_legacy_students)";
        $conn->query($sql);
        echo "Successfully updated Student Profile Picture to 'pix/anonymous.jpg' for " . $conn->affected_rows . " student accounts.\n";
        
        // Also update any other students in the database whose picture might be NULL or empty just to be thorough and cover 'all'
        $sql2 = "UPDATE student_info SET picture = 'pix/anonymous.jpg' WHERE picture IS NULL OR picture = ''";
        $conn->query($sql2);
        echo "Additionally updated " . $conn->affected_rows . " other accounts that had no picture.\n";
    }
