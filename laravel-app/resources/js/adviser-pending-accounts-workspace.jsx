...
function AdviserPendingAccountsWorkspace({ heading, description, actions = [], notes = [] }) {
    return (
        <section className="adviser-workspace adviser-workspace--cool">
            <div className="adviser-workspace__header">
                <div>
                    <div className="adviser-workspace__eyebrow">Account Review</div>
...
function mount(node) {
    const rawPayload = node.getAttribute('data-adviser-pending-accounts-workspace');
    if (!rawPayload) {
        return;
    }
    ...
    try {
        ...
        createRoot(node).render(<AdviserPendingAccountsWorkspace {...payload} />);
    } catch (error) {
        console.error('Unable to parse adviser pending accounts workspace payload.', error);
    }
}

document.querySelectorAll('[data-adviser-pending-accounts-workspace]').forEach(mount);
