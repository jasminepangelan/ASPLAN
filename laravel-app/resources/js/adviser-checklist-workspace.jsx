...
function AdviserChecklistWorkspace({ heading, description, actions = [], notes = [] }) {
    return (
        <section className="adviser-workspace">
            <div className="adviser-workspace__header">
                <div>
                    <div className="adviser-workspace__eyebrow">Checklist Tools</div>
                ...
                {actions.map((action) =>
                    action.href ? (
                        ...
                        <button
                            ...
                            onClick={() => runChecklistAction(action)}
                        >
                            <span className="adviser-workspace__action-title">{action.title}</span>
                            <span className="adviser-workspace__action-description">{action.description}</span>
                        </button>
...
function mount(node) {
    const rawPayload = node.getAttribute('data-adviser-checklist-workspace');
    if (!rawPayload) {
        return;
    }
    ...
    try {
        ...
        createRoot(node).render(<AdviserChecklistWorkspace {...payload} />);
    } catch (error) {
        console.error('Unable to parse adviser checklist workspace payload.', error);
    }
}

document.querySelectorAll('[data-adviser-checklist-workspace]').forEach(mount);
