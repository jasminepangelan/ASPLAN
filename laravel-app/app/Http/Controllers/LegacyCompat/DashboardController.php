...
class DashboardController extends Controller
{
    ...
    private function loadStudentShiftSummary(string $studentNumber): array
    {
        ...
        $row = DB::table('program_shift_requests')
            ...
            ->selectRaw("SUM(CASE WHEN status IN ('pending_current_coordinator', 'pending_destination_coordinator', 'pending_coordinator') THEN 1 ELSE 0 END) AS pending")
    ...
    private function loadAdviserShiftSummary(array $programKeys): array
    {
        ...
        $rows = DB::table('program_shift_requests')
            ...
            ->whereIn('status', ['pending_current_coordinator', 'pending_destination_coordinator', 'pending_coordinator', 'rejected'])
            ->get();
