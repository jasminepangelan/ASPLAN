...
class ChecklistController extends Controller
{
    ...
    public function save(Request $request): JsonResponse
    {
        try {
            ...
            return $this->saveStandard($request);
        } catch (Throwable $e) {
            return response()->json([
                'status' => 'error',
    ...
    private function saveStandard(Request $request): JsonResponse
    {
        $studentId = (string) $request->input('student_id', '');
        $courses = $this->parseArray($request->input('courses', []));
        ...
        DB::transaction(function () use ($studentId, $courses, $grades, $remarks, $professors, $timestamp, &$successful): void {
