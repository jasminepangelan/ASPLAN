<?php
/**
 * Normalize All Adviser Programs to Full Names
 * 
 * Standardizes all program variations in adviser table to match the full names
 * used in student_input_form_2.html and system mappings
 * 
 * Run this script through browser: http://localhost/ASPLAN_v5/dev/normalize_adviser_programs.php
 */
...
try {
    ...
    // Get all unique program values currently in the adviser table
    $checkQuery = "SELECT DISTINCT program FROM adviser WHERE program IS NOT NULL ORDER BY program ASC";
    ...
    echo json_encode([
        'status' => 'info',
        'message' => 'Current programs in adviser table:',
    ...
    foreach ($programNormalizationMap as $variation => $canonical) {
        $updateQuery = "UPDATE adviser SET program = ? WHERE TRIM(UPPER(program)) = UPPER(?)";
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Adviser program normalization completed',
        'total_variations_processed' => $updateCount,
        'total_records_updated' => $totalUpdated
    ]);
    
    // Final verification
    $verifyQuery = "SELECT DISTINCT program FROM adviser WHERE program IS NOT NULL ORDER BY program ASC";
    ...
    echo json_encode([
        'status' => 'info',
        'message' => 'Programmes after normalization:',
