...
class StudyPlanGenerator {
    ...
    // scheduleRemainingByPrereqChaining removed to ensure only CSP + Greedy algorithms are used.
    ...
    * - Back and failed course prioritization (lower years first)
    ...
    public function generateOptimizedPlan() {
        // Use CSP + Greedy algorithm only (basic/topological planner removed)
        ...
        // Note: prerequisite-chaining post-processing (topological/Kahn) was removed
        // to comply with the paper's requirement: use CSP + Greedy only.

        $this->plan_coverage = $this->calculatePlanCoverage($study_plan);
        return $study_plan;
    }

    // Basic/topological planner removed — generation uses CSP + Greedy only.
    ...
    private function determineCurrentTerm() {
        ...
        if ($latestSubmittedIndex >= 0) {
            ...
            // when those submitted grades are for the EXACT same term as the first
            // incomplete term. This allows the planner to advance past a term where
            // a student failed a subject, but prevents an erroneous massive jump 
            // when there are approved grades mis-mapped to a much later curriculum 
            // term (e.g., transfer/credited rows).
