...
$studentEmail = htmlspecialchars((string) ($_SESSION['student_email_verification_email'] ?? $_SESSION['email'] ?? ''), ENT_QUOTES, 'UTF-8');
$studentName = trim((string) (($_SESSION['first_name'] ?? '') . ' ' . ($_SESSION['last_name'] ?? '')));
$studentName = htmlspecialchars($studentName !== '' ? $studentName : (string) ($_SESSION['student_id'] ?? 'Student'), ENT_QUOTES, 'UTF-8');
$verificationNotice = trim((string) ($_SESSION['student_email_verification_notice'] ?? ''));
$autoSend = !empty($_SESSION['student_email_verification_autosend']);
unset($_SESSION['student_email_verification_autosend'], $_SESSION['student_email_verification_notice']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="verify-card">
        ...
        <p class="subtext">
            Hi <?php echo $studentName; ?>. Before you can access your student workspace, please confirm that your CvSU email address is real and reachable.
        </p>
        <div class="email-chip"><?php echo $studentEmail; ?></div>
        ...
        <p class="helper-note">
            We only require this extra step for student accounts using a <strong>@cvsu.edu.ph</strong> email address.
        </p>
    </div>
            ...
            postForm('verify_cvsu_email_otp.php', { code })
                .then(data => {
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.location.href = data.redirect || 'home_page_student.php';
                        }, 900);
                    } else {
